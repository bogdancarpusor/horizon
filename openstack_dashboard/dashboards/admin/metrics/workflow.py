...
class SelectResourceTypeAction(workflows.MembershipAction):
    def __init__(self, request, *args, **kwargs):
        ...
        self.fields[field_name].choices = [(0, 'a'), (1, 'b'), (2, 'c')]
...
class SelectResourceType(workflows.Workflow):
    slug = "select_resource_typer"
    name = _("Select Resource Type")
    finalize_button_name = _("Select")
    success_message = _('Selected new resource type "%s".')
    failure_message = _('Unable to select resource type "%s".')
...
class Resource(base.APIResourceWrapper):
    ...
    @property
    def name(self):
        name = self._apiresource.get('display_name', None)
    ...
    @property
    def meters(self):
        return self._apiresource.get('meters', None)
    ...
    def init_resources(gnocchiclient, resource_type, project_id):
        ...
        return [Resource(resource, gnocchiclient) for resource in resources 
                if str(resource['project_id']) == project_id]
    
    def get_metrics_dict(self, *args):
        pass
